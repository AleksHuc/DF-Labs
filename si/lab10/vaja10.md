...
Registra `edi` in `rsi` predstavljata argumenta funkcije `callme`. Prvi argument v registru `edi` zapišemo na naslov, ki je 4 bajte nad vrednostjo registra `rbp`. Drugi argument v registru `rsi` zapišemo na naslov, ki je 16 bajtov nad vrednostjo registra `rbp`. 
